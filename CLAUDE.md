...
- **ADR-051:** Engage RLS on the MCP path via AsyncLocalStorage + per-tx SET LOCAL ROLE atelier_runtime (option b in the S09 rebuild brief); defer supabase-js-on-MCP (option a) to v2-shape consideration. Closes the M8 grounding audit's S09 finding. Adds 33 `CREATE POLICY` statements per ARCH 5.3 covering 13 tables; helpers `atelier_current_composer_id()` / `atelier_current_project_id()` / `atelier_current_project_ids()` resolve the calling composer from `auth.jwt() ->> 'sub'`. The MCP request path enters an `AsyncLocalStorage` context once per `tools/call` (in `dispatch.ts` after `authenticate()` resolves AuthContext); `AtelierClient.tx()` / `poolStatement()` read the context and run `SET LOCAL ROLE atelier_runtime` + `set_config('request.jwt.claims', ...)` after `BEGIN` so RLS engages on every statement in the transaction. Sync paths (service_role) bypass via `FOR ALL TO service_role USING (true)` policies. Option (a) rejected for v1.x because the MCP write path uses fencing-CAS, advisory locks, pgvector custom operators, and deferred-broadcast tx patterns PostgREST does not naturally express; replicating those as SECURITY DEFINER RPCs is feasible but is a v2-shape change. Resolves BRD-OPEN-QUESTIONS §31 (S09 row). Migration: `supabase/migrations/20260508000016_atelier_rls_policies.sql`. Smoke: `scripts/endpoint/__smoke__/rls.smoke.ts` (14 checks).
- **ADR-052:** Cloudflare-primary infrastructure pivot. Compute/hosting/cron/edge primitives default to Cloudflare (Workers via `@opennextjs/cloudflare`, Cron Triggers, R2/KV/Queues/Durable Objects); Supabase Postgres+pgvector + Supabase Auth + Resend remain SaaS carve-outs (CF has no equivalent / SaaS is canonical pattern). Reverses ADR-046 (Vercel deploy strategy); amends ADR-027 capability mapping (Vercel cells flip to Cloudflare); amends ADR-029 (Cloudflare-portability becomes v1 constraint; GCP migration mapping preserved as v2 alternative). Forward-looking discipline: always default to Cloudflare; SaaS carve-out requires named disqualifier in the spec/ADR. Migration of the live Vercel deploy filed as BRD-OPEN-QUESTIONS §37 with trigger criteria. Surfaced 2026-05-09 by strategic call aligning Atelier with the wider-workspace pattern (`apps/photography`, `apps/zerospecs`, `apps/router`, `apps/blog`, `apps/letspepper`, `wip/simple-aes`, `wip/bcss/runtime-cf` all on Cloudflare); canonical-pattern-first rule had not been applied to the M6 Vercel deploy choice.
- **ADR-053:** Atelier and AI Hive (`bc-subscriptions/.hive/`) as paired canonical-vs-empirical substrates. Hive = empirical lab (BC-internal, hackathon-shipped, production-proven); Atelier = canonical destination (open market, self-hostable, spec-first). Bidirectional flow: Hive's load-bearing primitives flow into Atelier ADRs with `evidence: hive-production-use` justification; Atelier's architectural decisions inform Hive's roadmap when BC constraints permit. NOT a merge plan, NOT a deprecation of either, NOT a positioning of one over the other. The "Atelier vs Hive" framing was wrong-evidence; correct framing is paired-by-design. Discipline-tax memory note clarifies: applies within a single substrate, not across the pairing. Surfaced 2026-05-10 by first-principles audit triggered by BB's `hive-coordination-pattern.md` doc (2026-05-08) naming Hive as canonical pattern.
- **ADR-054:** Adopt brainstorm primitives from Hive (reverses BRD §35). Adds 6 tools (`propose`, `react`, `get_proposals`, `synthesize`, `approve_plan`, `checkpoint`) to Atelier's MCP surface. Tool surface lock expands 12→18; ADR-040 amended in lockstep. New schema: `proposals`, `proposal_reactions`, `syntheses`, `session_checkpoints` tables + `proposal_state` / `reaction_kind` enums + RLS per ADR-051 pattern. Brainstorm framed as structured deliberation (proposals MUST have title + body + options structure), NOT free-form chat — preserves PRD §5's "not a chat app" exclusion. First concrete instance of ADR-053's Hive→Atelier flow. Resolves BRD-OPEN-QUESTIONS §35.
- **ADR-055:** Three-loop coordination framing (brainstorm / execute / continuity) as canonical IA scaffolding. Borrowed from Hive's `ARCHITECTURE.md`. Atelier's 18 tools (post-ADR-054) organize as: brainstorm (5: propose/react/get_proposals/synthesize/approve_plan), execute (6: claim/update/release/acquire_lock/release_lock/propose_contract_change), continuity (7: register/heartbeat/deregister/checkpoint/log_decision/get_context/find_similar). Webapp v2 IA (Compose / Inbox / Activity / Atlas / Connect) integrates the three-loop axis as a layering. Concurrently: SSE push supersedes Supabase Realtime as canonical broadcast pattern (Hive's empirical 1s latency proof); Supabase Realtime adapter remains for M2-M7 era adopters; ADR-029's GCP-portability constraint preserved (SSE is HTTP standard). Strengthens ADR-052's CF-primary positioning (SSE native to Workers; Durable Object upgrade path documented for multi-isolate scale).
