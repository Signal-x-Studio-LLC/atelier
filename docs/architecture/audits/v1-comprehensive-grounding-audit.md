...
The M8 grounding audit's eleven `diverges-silently` findings are now closed. The audit's purpose — restoring methodological trust in the v1 substrate by empirically verifying every architectural seam against vendor canonicals — is achieved. The S09 v1.x follow-up (table-level RLS as second-layer defense + MCP path engaging RLS via `SET LOCAL ROLE atelier_runtime`) is closed by ADR-051 + `supabase/migrations/20260508000016_atelier_rls_policies.sql`. The supabase-js-on-MCP migration (option a in the rebuild brief) remains a v2-shape consideration in §31; option (b) preserves the pg.Pool path while closing the RLS gap.
...
| S09 RLS Policies | **closed** | Table-level `CREATE POLICY` statements added in `supabase/migrations/20260508000016_atelier_rls_policies.sql` per ARCH 5.3 (33 policies covering 13 tables); helper functions `atelier_current_composer_id()` / `atelier_current_project_id()` / `atelier_current_project_ids()` resolve the calling composer from `auth.jwt() ->> 'sub'`. MCP path engages RLS via `AsyncLocalStorage` + per-tx `SET LOCAL ROLE atelier_runtime` + `set_config('request.jwt.claims', ...)` per ADR-051; sync paths (service_role) bypass via `FOR ALL TO service_role USING (true)` policies. New `scripts/endpoint/__smoke__/rls.smoke.ts` (14 checks) verifies cross-composer write rejection at the Postgres tier and same-project read admittance. Ref: ADR-051. |
...
| S12 Webhook Handling | **closed** | All three inbound legs ship with HMAC verification + idempotency via `webhook_deliveries` (migration 13). **GitHub + Figma** at `prototype/src/app/api/webhooks/{github,figma}/route.ts` use raw HMAC-SHA256 (constant-time compare) per the GitHub canonical pattern. **Supabase Auth Hooks** at `prototype/src/app/api/webhooks/supabase-auth/route.ts` use Standard Webhooks / Svix-style verify via `prototype/src/lib/atelier/webhooks/svix-verify.ts` — `${id}.${timestamp}.${body}` HMAC, ±5min replay window, multi-version-tolerant signature header, dependency-free. **Per-event dispatch wired** for the GitHub + Figma legs (Track 3b): `push` on the project's default branch enqueues `embed_state` rows (migration 15) for paths under ADR/BRD/PRD/research per ARCH §6.4.2 / §902-905; `pull_request.closed && merged===true` marks the matching contribution `state=merged` (ARCH §716 authoritative path) via `repo_branch` lookup; Figma `FILE_COMMENT` inserts a `triage_pending` row routed via `ATELIER_FIGMA_{PROJECT_ID,TRIAGE_SESSION_ID,TRIAGE_TERRITORY_ID}` env (ARCH §6.5.2 / ADR-019). Smoke coverage: twenty-two checks for GitHub/Figma + fifteen for Supabase Auth (`prototype/__smoke__/webhooks{,-supabase-auth}.smoke.ts`) covering missing-headers, malformed signatures, wrong-version-prefix, stale-timestamp replay, valid first/duplicate, missing-secret fail-closed, GET 405, whsec_-prefixed-secret round-trip, plus end-to-end per-event side-effect verification. Supabase Auth Hooks per-event dispatch (auth.user.created → composer auto-provision; send-email → custom template) deferred to v1.x as adopter-signal-driven work — the spec gap S12 named (no verifying receiver) is closed across all three legs. |
