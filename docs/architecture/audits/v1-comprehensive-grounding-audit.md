...
| S12 Webhook Handling | **closed** | All three inbound legs ship: GitHub + Figma at `prototype/src/app/api/webhooks/{github,figma}/route.ts` (raw HMAC-SHA256, constant-time compare); Supabase Auth Hooks at `prototype/src/app/api/webhooks/supabase-auth/route.ts` (Standard Webhooks/Svix-style verify via `prototype/src/lib/atelier/webhooks/svix-verify.ts` — `${id}.${timestamp}.${body}` HMAC, ±5min replay window, multi-version-tolerant signature header, dependency-free). Idempotency shared via `webhook_deliveries` table (migration 13) keyed on the provider's per-delivery ID. Smoke coverage: 11 checks for GitHub/Figma + 15 for Supabase Auth (`prototype/__smoke__/webhooks-supabase-auth.smoke.ts`) covering missing-headers (400/401), malformed signature (401), wrong-version-prefix (401), stale-timestamp replay (401), valid first/duplicate (200, idempotent on second), missing-secret fail-closed (500), GET 405, whsec_-prefixed-secret round-trip. Per-event dispatch (push → embed pipeline; pull_request.merged → contribution state; FILE_COMMENT → triage; auth.user.created → composer auto-provision; send-email → custom template) deferred to v1.x — the spec gap S12 named (no verifying receiver) is closed across all three legs; per-event handler wiring is incremental adopter-signal-driven work. |
