...
3. **Fail-safe is load-bearing, not advisory.** The default treats `MANUAL_REVIEW` as non-blocking (so a stale artifact does not wedge the flow) but never silently green — it warns and refuses to report COMPLIANT. **Enforcement posture (revised once the catalog reflected the real surface): repo-level CI runs PLAIN, per-AC runs `--strict`.** The `fast` job of `atelier-audit.yml` regenerates the artifact and runs plain `atelier dod` — it blocks on NON-COMPLIANT (a capability that should ship but doesn't) and surfaces `MANUAL_REVIEW` without blocking. Repo-level `--strict` is wrong now that the catalog honestly carries `manual_review` capabilities (e.g. epic-8 contracts, stubbed at M2 per ADR-040): it would block every unrelated PR on an accepted stub. `--strict` belongs on the per-contribution path — `atelier dod --trace-id BRD:Epic-N --strict` — where a contribution gates on its own AC's completion.
