...
1. **Cron blocker resolution.** Vercel Hobby plan limits cron schedules; M8 surfaced this as a live operator-decision blocker (matrix S03 → BRD §28 follow-up). Cloudflare Cron Triggers ship on the free tier with arbitrary schedules; the blocker disappears for free.
...
- Vercel Hobby cron blocker (matrix S03 → operator-pending follow-up) resolves for free under Cloudflare Cron Triggers; the operator-decision tree (upgrade Pro / relax crons / migrate) collapses to "execute the CF migration."
