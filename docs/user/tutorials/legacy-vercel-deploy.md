# Legacy: First deploy on Vercel + Supabase Cloud

> **DEPRECATED 2026-05-09 per ADR-052.** Cloudflare Workers + Supabase Cloud is the canonical reference deploy going forward. New adopters should follow `cloudflare-bootstrap.md` instead. This runbook is retained for: (a) adopters running an existing Vercel deploy who haven't cut over yet (BRD-OPEN-QUESTIONS §37 PR 4 cutover is the trigger), (b) adopters who specifically choose Vercel with a documented "why not Cloudflare" rationale per ADR-052's discipline rule.
